  ...
  return (
    <main className="font-sans bg-[#F5F3EC] text-[#1A1A1A]">
      ...
      <section className="pt-28 pb-32 px-6 relative overflow-hidden">
        ...
        >
          <div className="lg:w-[55%] space-y-8">
            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="inline-flex items-center gap-3 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-200">
              <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
              <p className="text-xs font-bold tracking-widest uppercase text-gray-600">Premium Grooming di Sangatta</p>
            </motion.div>

            <motion.h1 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="text-6xl lg:text-[5.5rem] font-black leading-[0.9] tracking-tighter uppercase text-[#1A1A1A]">
              Gaya Rambut <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#1A1A1A] to-gray-400">Terbaik</span> <br />
              Di Kota Ini.
            </motion.h1>

            <motion.p variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="text-gray-600 text-lg max-w-md leading-relaxed font-medium">
              Kami fokus pada kenyamanan, presisi, dan detail untuk menciptakan gaya yang membuatmu tampil penuh percaya diri.
            </motion.p>

            <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="pt-4 flex items-center gap-6">
          ...
          >
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 1.1 }} className="absolute -bottom-8 -left-8 bg-white p-5 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.08)] flex items-center gap-4">
              <div className="text-yellow-500 flex text-xl">★★★★★</div>
      ...
      <section className="relative z-20 max-w-7xl mx-auto px-6 -mt-10 lg:-mt-16 mb-24">
        ...
        >
          <motion.div variants={{ hidden: { opacity: 0, y: 40 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="bg-white p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-gray-100 flex flex-col gap-4 group hover:-translate-y-2 transition-all duration-300">
          ...
          <motion.div variants={{ hidden: { opacity: 0, y: 40 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="bg-[#1A1A1A] text-white p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex flex-col justify-between group hover:-translate-y-2 transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            {daftarLayanan.map((layanan) => (
              <motion.div key={layanan.id} variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5 } } }} className="group bg-white p-8 rounded-3xl border border-gray-200 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full">
                <div>
                ...
                <div className="flex items-end justify-between mt-auto pt-4 border-t border-gray-100">
                  ...
                  <Link href="/reservasi" className="w-10 h-10 rounded-full border border-gray-300 flex items-center justify-center text-gray-500 group-hover:bg-[#1A1A1A] group-hover:text-white group-hover:border-[#1A1A1A] transition-colors">↗</Link>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      <section id="gallery" className="bg-white py-32 border-t border-gray-200 overflow-hidden relative">
        <style dangerouslySetInnerHTML={{
          __html: `
          @keyframes scroll-infinite { 0% { transform: translateX(0); } 100% { transform: translateX(calc(-100% - 24px)); } }
          .animate-scroll-infinite { animation: scroll-infinite 35s linear infinite; }
          .gallery-container:hover .animate-scroll-infinite { animation-play-state: paused; }
        `}} />
        ...
        >
          <div className="mb-16">
            <p className="text-xs font-bold tracking-widest uppercase text-gray-500 mb-2 flex items-center gap-3">
              <span className="w-2 h-2 rounded-full bg-gray-300"></span> Inside Look
            </p>
        ...
        >
          ...
          <div className="flex gap-6 pr-6 animate-scroll-infinite w-max">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div key={`grup1-${item}`} className="shrink-0 w-[280px] md:w-[350px] aspect-[4/5] bg-gray-100 rounded-[2rem] overflow-hidden relative group cursor-pointer">
                <img src={`/images/model_${item}.jpg`} alt={`Model ${item}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
            ))}
          </div>
          <div className="flex gap-6 pr-6 animate-scroll-infinite w-max" aria-hidden="true">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div key={`grup2-${item}`} className="shrink-0 w-[280px] md:w-[350px] aspect-[4/5] bg-gray-100 rounded-[2rem] overflow-hidden relative group cursor-pointer">
                <img src={`/images/model_${item}.jpg`} alt={`Model ${item}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
            ))}
          </div>
        ...
        >
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between border-b border-gray-800 pb-16 mb-16 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-5xl md:text-7xl font-black tracking-tighter uppercase leading-none mb-6">SIAP TAMPIL <br /><span className="text-gray-600">MAKSIMAL?</span></h2>
              <p className="text-gray-400 text-sm max-w-md leading-relaxed">Jangan biarkan rambut berantakan merusak harimu. Amankan kursimu sekarang dan rasakan pengalaman cukur premium di Sangatta.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 text-sm text-gray-400 mb-20">
            <div className="space-y-4"><h4 className="text-white font-black uppercase tracking-widest text-xs">Lokasi Kami</h4><p className="leading-relaxed">Jl. APT Pranoto<br />Belakang Indomaret<br />Sangatta Utara, Kutai Timur</p></div>
            <div className="space-y-4"><h4 className="text-white font-black uppercase tracking-widest text-xs">Kontak</h4><p className="leading-relaxed">0857-5342-4792<br />web.soedimampir@gmail.com</p></div>
            <div className="space-y-4"><h4 className="text-white font-black uppercase tracking-widest text-xs">Jam Operasional</h4><p className="leading-relaxed">Setiap Hari<br />13:00 - 23:00 WITA</p></div>
            <div className="space-y-4"><h4 className="text-white font-black uppercase tracking-widest text-xs">Sosial Media</h4><div className="flex gap-4"><a href="https://www.instagram.com/soedi_mampir/" className="hover:text-white transition-colors underline underline-offset-4">Instagram</a><a href="https://wa.me/6285753424792" className="hover:text-white transition-colors underline underline-offset-4">WhatsApp</a></div></div>
          </div>
